...
public class StreamSound extends SoundParent{

	final int EXTERNAL_BUFFER_SIZE = 128;
	private SoundThread soundThread;
	private String loc;
	private boolean garbageCollect = true;
	public StreamSound(String location){
		create(location);
	}
	public StreamSound(String location, double scale){
		create(location);
		setVolume(scale);
	}
	public StreamSound(String location, boolean garbageCollect){
	...
	private class SoundThread implements Runnable{
		...
		private void createStream(String location){
			InputStream is = getClass().getClassLoader().getResourceAsStream("Data/" + location);
		...
		private void createLine(){
			...
			Sound.addToVolumes(volume, lastScale);
			setVolume(lastScale);
		}
		@Override
		public void run() {
		...
		public synchronized void resetPlaying(){
			...
			Sound.removeFromVolumes(volume);
			createStream(loc);
			createLine();
		}
		public synchronized void setVolume(double scale){
			lastScale = scale;
			Sound.changeVolume(volume, scale);
			volume.setValue((float) ((Sound.masterScale * scale)*(Math.abs(volume.getMinimum()) + Math.abs(volume.getMaximum())) + volume.getMinimum()));
		}
